...
import ShopCraftAIChat from '../components/dashboard/ShopCraftAIChat';
    ...
    <div className="p-10 max-w-7xl mx-auto">
      <header className="mb-16 flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
        ...
        <div className="flex flex-wrap gap-4">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-2xl bg-indigo-600 px-5 py-3 text-xs font-black uppercase tracking-widest text-white shadow-xl shadow-indigo-500/20 transition-all hover:bg-indigo-500"
          >
            <Plus size={16} />
            Create Website
          </button>
      ...
      <section className="mb-20">
        <div className="flex items-center justify-between mb-10">
          ...
          <button onClick={() => navigate('/dashboard/projects')} className="text-indigo-400 hover:text-white transition-colors font-black uppercase tracking-widest text-xs">View All Projects</button>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            { label: 'Recent Projects', icon: Clock, count: projects.length, path: '/dashboard/projects' },
            { label: 'Saved Templates', icon: Layout, count: 0, path: '/dashboard/templates' },
            { label: 'Favorite Projects', icon: Star, count: projects.filter((project) => project.favorite).length, path: '/dashboard/favorites' },
            { label: 'Saved 2D Designs', icon: Palette, count: projects.filter((project) => project.type === 'design2d').length, path: '/builder/design-2d' },
            { label: 'Saved 3D Visuals', icon: Box, count: projects.filter((project) => project.type === 'model3d').length, path: '/builder/model-3d' },
            { label: 'Published Websites', icon: Globe, count: projects.filter(p => p.status === 'published').length, path: '/dashboard/published' },
          ].map((cat, i) => (
            <motion.div 
              key={cat.label}
              ...
              tabIndex={0}
              onKeyDown={(event) => {
                if (event.key === 'Enter' || event.key === ' ') navigate(cat.path);
              }}
      ...
      <section>
        ...
        {projects.length > 0 ? (
          ...
          <div className="bg-slate-900/30 border border-dashed border-slate-800 rounded-[3rem] p-20 text-center">
            ...
            <button onClick={() => navigate('/create/website')} className="bg-indigo-600 px-10 py-4 rounded-2xl text-white font-black uppercase tracking-widest text-sm hover:bg-indigo-500 transition-all shadow-xl shadow-indigo-500/20">

...
export default function WebsiteCanvas({ readonly = false, runtimeMode = 'editor' }) {
  ...
  const handleDragEnd = (event) => {
    ...
    if (activeData.dragType === 'new-element') {
      addElement(activeData.elementType, overData.sectionId);
      return;
    }

    if (activeData.dragType === 'new-section') {
      addSection(activeData.sectionType);
      return;
    }

    if (activeData.dragType === 'section' && overData.dragType === 'section') {
      reorderSections(activeData.sectionId, overData.sectionId);
      return;
    }

    if (activeData.dragType === 'element') {
      reorderElements(activeData.elementId, activeData.sectionId, overData.elementId, overData.sectionId);
      return;
    }
  ...
  const handleNativeDrop = (event) => {
    ...
    try {
      ...
      if (data.dragType === 'new-section') addSection(data.sectionType);
      if (data.dragType === 'new-element') addElement(data.elementType);
    } catch {
            ...
            <SortableContext items={sections.map((section) => section.id)} strategy={verticalListSortingStrategy}>
              ...
              {sections.map((section, index) => (
                <React.Fragment key={section.id}>
                  <SectionRenderer section={section} index={index} readonly={readonly} device={activeDevice} runtimeMode={runtimeMode} />
            ...
            <div className="pointer-events-none flex min-h-[72vh] flex-col items-center justify-center p-10 text-center text-slate-500">
              ...
              {!readonly && (
                <div className="pointer-events-auto flex flex-wrap justify-center gap-3">
                  <button type="button" onClick={() => addSection('navbar')} className="inline-flex items-center gap-2 rounded-xl bg-slate-950 px-5 py-3 text-xs font-black uppercase tracking-widest text-white">
                    <Plus size={16} />
                    Add Navbar
                  </button>
                  <button type="button" onClick={() => addSection('hero')} className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-5 py-3 text-xs font-black uppercase tracking-widest text-white">
                    <Plus size={16} />
                    Add Hero Section
                  </button>
                  <button type="button" onClick={() => addSection('custom')} className="inline-flex items-center gap-2 rounded-xl border border-slate-300 px-5 py-3 text-xs font-black uppercase tracking-widest text-slate-700 hover:border-indigo-500">
                    Blank Section
                  </button>
                  <button type="button" onClick={() => addSection('footer')} className="inline-flex items-center gap-2 rounded-xl border border-slate-300 px-5 py-3 text-xs font-black uppercase tracking-widest text-slate-700 hover:border-indigo-500">
                    Add Footer
                  </button>
